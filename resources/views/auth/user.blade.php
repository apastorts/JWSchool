                    ...
                    <form method="POST" action="/profile/update/{{ $user->id  }}">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-6">
                                <select id="sex" type="text" class="form-control" name="sex">
                                    <option value="male" {{ $user && $user->sex == 'male' ? 'selected' : '' }}>{{ __('Male') }}</option>
                                    <option value="female" {{ $user && $user->sex == 'female' ? 'selected' : '' }}>{{ __('Female') }}</option>
                                </select>
                            </div>
                        </div>
                            ...
                            <div class="col-md-6">
                                <select id="role" type="text" class="form-control" name="role" >
                                  @foreach($roles as $role)
                                    <option value="{{ $role->id }}" {{ $user &&  $role->id == $user->role->id ? 'selected' : '' }}>{{ $role->name }}</option>
                                  @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="form-group row mb-0">
                            <div class="col-md-6 offset-md-4">
                                <button type="submit" class="main-button p-2 rounded">
                                    {{ __('Editar') }}
                                </button>
                            </div>
                        </div>
